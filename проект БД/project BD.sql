-- база данных медицинского стационара,в котором есть палаты для больных.У каждого отделения есть своя единственная палата.
-- в БД присутствуют две таблицы с личными данными(user_profiles- данные врачей и patient_profiles-данные пациентов).
-- также присутствуют справочники : должностей,филиалов,отделений,лекарств и диагнозов.
-- все данные из справочников и профилей врачей и пациентов записываются в Электронной Медицинской Карте пациента.
-- Пациента можно разместить в палате на какой либо срок.
-- есть расписание(какой пациент к какому врачу и на какое время записан).
